...
async def get_or_create_country(country_code, db: AsyncSession):
    ...
    if not country_obj:
        country_schema = schemas.CountryInSchema(code=country_code, name=None)
...
async def get_or_create_genre(genre_name, db: AsyncSession):
    genre_obj = await db.scalar(select(GenreModel).where(GenreModel.name == genre_name))
    if not genre_obj:
        genre_schema = schemas.GenreInSchema(name=genre_name)
        genre_obj = await create_genre(db, genre_schema)
        return genre_obj
    else:
        return genre_obj
...
async def get_or_create_actor(actor_name, db: AsyncSession):
    ...
    if not actor_obj:
        actor_schema = schemas.ActorInSchema(name=actor_name)
...
async def get_or_create_language(language_name, db: AsyncSession):
    ...
    if not language_obj:
        language_schema = schemas.LanguageInSchema(name=language_name)
...
async def patch_movie(
    db_movie: MovieModel, db: AsyncSession, movie: schemas.MovieUpdateInSchema
):
